...
**Xadarr is fully standalone** — point it at any IPTV M3U URL, a Jellyfin/Plex/Emby server, or nothing at all. Dispatcharr and Episeerr are Joe's own infrastructure layered on top; none of their integrations are required for basic use.
...
### 17. IPTV Group Management (Dispatcharr integration)
Three-state per group: **Show / Hide / Remove**. New groups that appear in an M3U sync are automatically hidden and badged "NEW" until the user explicitly shows them. Groups marked Remove are written to a configurable blacklist file for Dispatcharr to consume.

- **`GroupState` enum** in `IptvModels.kt`: `Show`, `Hide`, `Remove`
- **`IptvSnapshot`** carries `newGroups: List<String>` and `removedGroups: List<String>` alongside existing `hiddenGroups`
- **`knownGroups` migration**: first run after upgrade seeds all current groups as known (no mass auto-hide); subsequent M3U loads auto-hide new unknown groups into `newGroups`
- **`setGroupState()`** in `IptvRepository`: removes group from all three sets, places it in the target set, writes the blacklist file on Remove/Show transitions
- **`writeBlacklistFile()`**: reads `DISPATCHARR_BLACKLIST_PATH_KEY`, writes one removed group name per line; silently skips if path is blank
- **`DISPATCHARR_BLACKLIST_PATH_KEY`** in `WebhookRepository.kt`; configurable in Settings → Plugins & Extensions → DISPATCHARR section; default path `/data/dispatcharr_blacklist.txt`
- **`LiveCategoryTree`** gains `removed: LiveSection`; `buildCategoryTree` accepts `newGroups` and `removedGroups` sets and routes groups into the correct section
- **`CategorySidebar` edit mode**: internal `groupEditMode` boolean (not in parent state). "Edit groups" row at bottom of PLAYLIST section toggles it. In edit mode: HIDDEN section shows all hidden groups with visual distinction (amber "NEW" badge + `FiberNew` icon for new groups; `VisibilityOff` for manually hidden); REMOVED section shows removed groups with `Block` icon and "Restore group" action
- **Context menus**: PLAYLIST groups get "Hide group" + "Remove group" + move options; HIDDEN groups get "Show group" + "Remove group"; REMOVED groups get "Restore group" only
- **Dispatcharr `maintenance.sql` Part 4**: reads the blacklist file via `\copy FROM PROGRAM 'cat /blacklist.txt'` and deletes `auto_created` channels belonging to blacklisted groups. Requires volume mount: `/home/joe/config/xadarr-server/data/dispatcharr_blacklist.txt:/blacklist.txt:ro` in the dispatcharr container
...
| `data/model/IptvModels.kt` | `GroupState` enum, `IptvSnapshot.newGroups`/`removedGroups` |
| `/home/joe/config/dispatcharr/scripts/maintenance.sql` | Dispatcharr channel maintenance — group mapping, whitelist, merge, numbering (outside project tree) |
...
**Tier 1 — Daily watching (~377 channels)**
...
MLB Live Pass, NFL Game Pass, UFC Fight Pass, Peacock, Paramount+, UK Sky Sports, World Cup, PPV events, and other event-based groups. The maintenance script leaves these completely untouched — they stay in their provider source group names. Xadarr auto-hides new groups on M3U sync (they appear in the TV guide HIDDEN section with a NEW badge). Channels with matching tvg_ids across Tier 2 groups are consolidated by the script (Step E) for provider failover, same as Tier 1.

To watch an event: open TV guide → D-pad left to category sidebar → long-press "Edit groups" → HIDDEN section appears → unhide the relevant group. Hide it again when done.
...
**Script parts (in execution order):**
- **Part 3** — Source group → target group mapping (Tier 1 only; Tier 2 groups stay in their original group names so Xadarr can auto-hide them)
...
- **Part 8** — Channel numbering (runs last, after all merges and deletes)
...
- **Fix: xadarr-server IPTV fallback** — `GET /api/iptv` falls back to root-level `iptvM3uUrl`/`iptvEpgUrl` when per-profile entry absent
- **Fix: xadarr-server connection preservation** — no-token sync payloads can't overwrite existing home server connections
- **Sync server URL triggers immediate pull** — entering the sync server URL pulls cloud state immediately (IPTV, watchlist, settings) on new devices
- **xadarr-server: smart server display name** — Home rows use `serverFriendlyName()`: displayName → serverName → server type
- **xadarr-server: reconnect button** — server cards have ↺ to re-auth without removing; Cancel resets form

Changes since v2.6.2 (v2.7):
- Three-state group management in TV guide sidebar: Show / Hide / Remove (replaces binary toggle)
- New groups auto-hidden on first M3U sync and badged "NEW"; existing groups on upgrade are silently migrated (no mass-hide)
- Groups marked Remove are written to a configurable blacklist file (Settings → Plugins & Extensions → DISPATCHARR)
- `CategorySidebar` edit mode (long-press "Edit groups"): shows hidden + removed sections with visual distinction
- `maintenance.sql` Part 4 reads the blacklist file via `\copy FROM PROGRAM` and deletes blacklisted group channels from Dispatcharr
- Blacklist file default path: `/data/dispatcharr_blacklist.txt`; requires `:/blacklist.txt:ro` volume mount in dispatcharr container
- **Dispatcharr Bridge addon** — Dispatcharr feature is addon-gated
- **SecureStorage decrypt fix** — `getExistingKey()` never creates a new key during decrypt
- **Stable APK signing** — sideload debug builds use release keystore when `keystore.properties` present

## Addon Catalogue Behaviour (important)

Stremio addons only create catalogue rows if their `manifest.json` declares non-empty `catalogs`. Most stream-only addons (Torrentio, YasStreams) have `"catalogs": []` — they contribute stream sources to the stream picker but produce **no home/discover rows**. Addons that DO declare catalogs (AIOStreams with metadata enabled, home server addons) get rows auto-created by `syncAddonCatalogs()` on the next Home screen load.

The **dispatcharr-bridge** addon (`http://192.168.254.205:7979/dispatcharr-bridge`) is served as a static file from xadarr-server's `web/dispatcharr-bridge/manifest.json`. It has `"catalogs": []` and `"resources": []` — purely a marker addon. Its manifest contains a custom `"xadarr": { "extensions": ["group_blacklist"], "groupBlacklist": { "defaultPath": "/data/dispatcharr_blacklist.txt" } }` field that Xadarr does **not yet read**. Future work: parse this field on addon install to auto-configure the blacklist path in Settings.

**Add Catalogue** (Settings → Catalogs) is separate from addon catalogs — it's for manually adding Trakt list URLs or MDBList URLs as custom catalogue rows. Nothing to do with Stremio addons.
